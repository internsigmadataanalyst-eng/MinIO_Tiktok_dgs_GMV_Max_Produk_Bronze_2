# src/gmv_max_produk/utils/watermark_monitor.py
...
Informational only: the ETL proceeds as long as there are no access errors
(Gate 1). The BEHIND/OK report here drives the downstream per-sheet watermark
filter, which only picks up rows newer than each sheet's watermark — so sheets
...
def gmv_max_produk_watermark_check() -> pd.DataFrame:
    """Check watermark drift for the 5 GMV Max Produk sheets (matz..imam) using toko grain."""
    watermark_records = get_watermark(
        bucket="tiktok-dgs-gmvmax-produk-bronze",
        watermarks_path="watermarks/gmv_max.json",
    )
    sheet_registry = {
        "matz": ("SH_KEY_MATZ", "GMV MAX Produk"),
        "ian":  ("SH_KEY_IAN",  "GMV MAX Produk"),
        "deni": ("SH_KEY_DENI", "GMV MAX Produk"),
        "riwa": ("SH_KEY_RIWA", "GMV MAX Produk"),
        "imam": ("SH_KEY_IMAM", "GMV MAX Produk"),
    }
    registry_keys = list(sheet_registry.keys())
    return compare_watermark_vs_sheet(
        "GMV Max Produk", registry_keys, sheet_registry, watermark_records,
        toko_col="toko", tanggal_col="tanggal",
        required_cols=["id_campaign"],
    )