...
from src.gmv_max_produk.utils.log import emit

# .env lives at the package root (GMV_Max_produk/.env), three levels up.
...
def build_pipeline_success_email(
    ...
    """
    subject = f"[ETL NOTIFY] Pipeline SUCCESS — {pipeline_name}"
    parts = []
    if status:
...
def build_pipeline_failure_email(
    ...
    """
    subject = f"[ETL ALERT] Pipeline FAILED — {pipeline_name}"
    parts = [
...
def build_gate_abort_email(
    ...
    """
    subject = f"[ETL ALERT] ETL aborted — pre-flight gate {gate} rejected"
...
QUARANTINE_SAMPLE_COLUMNS = ["sheet_name", "Tanggal", "Toko", "ID Campaign", "error_reason"]
...
def build_quarantine_email(
    ...
    """
    subject = f"[ETL ALERT] Quarantine — {n_quarantined} bad row(s) quarantined"
